...
@Slf4j
@RestController
public class BR_CM_ROLE_CD_SAVE {

	@JsonRootName("IN_DS")
	@ApiModel(value="IN_DS-BR_CM_ROLE_CD_SAVE")
	@Data
	static class IN_DS {
		@JsonProperty("brRq")
		...
		@Schema(name="IN_DATA-BR_CM_ROLE_CD_SAVE", description = "입력 데이터")
		ArrayList<DATA_ROW> IN_DATA = new ArrayList<DATA_ROW>();
		
		@JsonProperty("UPDT_DATA")
		@Schema(name="IN_DATA-BR_CM_ROLE_CD_SAVE", description = "수정 데이터")
	...
	@ApiModel(value="DATA_ROW-BR_CM_ROLE_CD_SAVE")
	@Data
	...
	@JsonRootName("OUT_DS")
	@ApiModel(value="OUT_DS-BR_CM_ROLE_CD_SAVE")
	@Data
	static class OUT_DS {
	...
	@Autowired
	DA_CM_ROLE_CD daCmRoleCd;

	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "successful operation", content = {
			@Content(mediaType = "application/json", schema = @Schema(implementation = OUT_DS.class)) }) 
	})
	@ApiOperation(tags={"CM_ROLE_CD"},value = "역할코드를 저장한다.", notes = "")
	@PostMapping(path= "/api/BR_CM_ROLE_CD_SAVE", consumes = "application/json", produces = "application/json")
	public OUT_DS run(@RequestBody IN_DS inDS) throws BizException {
		...
		for( int i=0;i<inDS.IN_DATA.size();i++) {
			...
			String  ROLE_NM 	= PjtUtil.str(rs.ROLE_NM);
			...
			if(PjtUtil.isEmpty(ROLE_NM)) {
				throw new BizRuntimeException("역할명이 입력되지 않았습니다.");
			}
