...
@OpService
@Service
public class BR_UPBIT_CANDLES_MONTHS_FIND {

	@Autowired
    PjtUtil pjtU;
	
	@JsonRootName("IN_DS")
	@ApiModel(value="IN_DS-BR_UPBIT_CANDLES_MONTHS_FIND")
	@Data
	static class IN_DS {
		@JsonProperty("brRq")
		...
		@Schema(name="IN_DATA-BR_UPBIT_CANDLES_MONTHS_FIND", description = "입력 데이터")
	...
	@ApiModel(value="IN_DATA_ROW-BR_UPBIT_CANDLES_MONTHS_FIND")
	@Data
	...
	@JsonRootName("OUT_DS")
	@ApiModel(value="OUT_DS-BR_UPBIT_CANDLES_MONTHS_FIND")
	@Data
	static class OUT_DS {
		@JsonProperty("OUT_DATA")
		@Schema(name="OUT_DATA-BR_UPBIT_CANDLES_MONTHS_FIND", description = "출력 데이터")
		ArrayList<OUT_DATA_ROW> OUT_DATA = new ArrayList<OUT_DATA_ROW>();

		@JsonProperty("PAGE_DATA")
		@Schema(name="PAGE_DATA-BR_UPBIT_CANDLES_MONTHS_FIND", description = "페이지 데이터")
		PAGE_DATA_ROW PAGE_DATA;
	}

	@ApiModel(value="OUT_DATA_ROW-BR_UPBIT_CANDLES_MONTHS_FIND")
	@Data
	static class OUT_DATA_ROW {
		...
		@Schema(name = "FIRST_DAY_OF_PERIOD", example = "0.0208284024", description = "캔들 기간의 가장 첫 날")
		String FIRST_DAY_OF_PERIOD = null;
	...
	public OUT_DS run(@RequestBody IN_DS inDS) throws BizException {
		...
		Page<Tuple>  pg=daUpbitCandlesMonths.find(p,SEARCH_NM,MARKET_WARNING,MARKET_CD);
		...
		for (int i = 0; i < al.size(); i++) {
			...
			row.CANDLE_DATE_TIME_UTC = c.get(QUpbitCandlesMonths.upbitCandlesMonths.candleDateTimeUtc);
			row.CANDLE_DATE_TIME_KST = c.get(QUpbitCandlesMonths.upbitCandlesMonths.candleDateTimeKst);
			row.OPENING_PRICE =  c.get(QUpbitCandlesMonths.upbitCandlesMonths.openingPrice);
			row.HIGH_PRICE =  c.get(QUpbitCandlesMonths.upbitCandlesMonths.highPrice);
			row.LOW_PRICE =  c.get(QUpbitCandlesMonths.upbitCandlesMonths.lowPrice);
			row.TRADE_PRICE =  c.get(QUpbitCandlesMonths.upbitCandlesMonths.tradePrice);
			row.TIMESTAMP = c.get(QUpbitCandlesMonths.upbitCandlesMonths.timestamp);
			row.CANDLE_ACC_TRADE_PRICE = c.get(QUpbitCandlesMonths.upbitCandlesMonths.candleAccTradePrice);
			row.CANDLE_ACC_TRADE_VOLUME = c.get(QUpbitCandlesMonths.upbitCandlesMonths.candleAccTradeVolume);
			row.FIRST_DAY_OF_PERIOD = c.get(QUpbitCandlesMonths.upbitCandlesMonths.firstDayOfPeriod);

			row.UPDT_DTM = pjtU.getYyyy_MM_dd_HHMMSS(c.get(QUpbitCandlesMonths.upbitCandlesMonths.updtDtm));
			row.CRT_DTM = pjtU.getYyyy_MM_dd_HHMMSS(c.get(QUpbitCandlesMonths.upbitCandlesMonths.crtDtm));
