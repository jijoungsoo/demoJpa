...
@OpService
@Service
public class BR_UPBIT_EXCHANGE_POST_ORDERS_SELL {

	@Autowired
    PjtUtil pjtU;
	
	@JsonRootName("IN_DS")
	@ApiModel(value="IN_DS-BR_UPBIT_EXCHANGE_POST_ORDERS_SELL")
	@Data
	static class IN_DS {
		@JsonProperty("brRq")
		...
		@Schema(name="IN_DATA-BR_UPBIT_EXCHANGE_POST_ORDERS_SELL", description = "입력 데이터")
		ArrayList<IN_DATA_ROW> IN_DATA = new ArrayList<IN_DATA_ROW>();
	}

	@ApiModel(value="IN_DATA_ROW-BR_UPBIT_EXCHANGE_POST_ORDERS_SELL")
	@Data
	...
	@JsonRootName("OUT_DS")
	@ApiModel(value="OUT_DS-BR_UPBIT_EXCHANGE_POST_ORDERS_SELL")
	@Data
	static class OUT_DS {
	...
	@ApiModel(value="OUT_DATA_ROW-BR_UPBIT_EXCHANGE_POST_ORDERS_SELL")
	@Data
	...
	public OUT_DS run(@RequestBody IN_DS inDS) throws BizException {
		...
		if(ORD_TYPE.toUpperCase().equals("MARKET")||    /* market : 시장가 주문(매도) */
				ORD_TYPE.toUpperCase().equals("LIMIT") /*limit : 지정가 주문 */
		){
			//정상
		} else {
			//비정상
			throw new BizException("매수시 ord_type은 limit나 market 이여야 합니다.:"+ORD_TYPE);
		}
		...
		Double d_min_total = Double.parseDouble(tmp_out.getOUT_DATA().get(0).getMARKET__ASK__MIN_TOTAL());
		...
		if(ORD_TYPE.toUpperCase().equals("PRICE") ){/* price : 시장가 주문(매수) */
			...
			if(d_min_total>d_price){
				throw new BizException("시장가 매도:: 최소 주문금액보다 주문금액이 작습니다. (최소주문금액:"+d_min_total+", 주문금액:"+d_price +")");
			}
		}
		if(ORD_TYPE.toUpperCase().equals("LIMIT") ){/* limit : 지정가 */
			...
			if(d_min_total>(d_price*d_volume)){
				throw new BizException("지정가 매도:: 최소 주문금액보다 주문금액이 작습니다. (최소주문금액:"+d_min_total+", 주문금액:"+(d_price*d_volume) +", 금액:"+d_price+",주문량:"+d_volume+")");
			}
		}
