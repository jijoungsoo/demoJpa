...
import com.example.demo.sa.upbit.api.SA_UPBIT_EXCHANGE_GET_ORDER;
...
@OpService
@Service
public class BR_UPBIT_EXCHANGE_GET_ORDER {

	@Autowired
    PjtUtil pjtU;
	
	@JsonRootName("IN_DS")
	@ApiModel(value="IN_DS-BR_UPBIT_EXCHANGE_GET_ORDER")
	@Data
	static class IN_DS {
		@JsonProperty("brRq")
		...
		@Schema(name="IN_DATA-BR_UPBIT_EXCHANGE_GET_ORDER", description = "입력 데이터")
		ArrayList<IN_DATA_ROW> IN_DATA = new ArrayList<IN_DATA_ROW>();
	}

	@ApiModel(value="IN_DATA_ROW-BR_UPBIT_EXCHANGE_GET_ORDER")
	@Data
	...
	@JsonRootName("OUT_DS")
	@ApiModel(value="OUT_DS-BR_UPBIT_EXCHANGE_GET_ORDER")
	@Data
	static class OUT_DS {
		@JsonProperty("OUT_DATA")
		@Schema(name="OUT_DATA-BR_UPBIT_EXCHANGE_GET_ORDER", description = "출력 데이터")
		ArrayList<OUT_DATA_ROW> OUT_DATA = new ArrayList<OUT_DATA_ROW>();

		
		@JsonProperty("OUT_DATA_TRADES")
		@Schema(name="OUT_DATA_TRADES-BR_UPBIT_EXCHANGE_GET_ORDER", description = "출력 데이터")
		ArrayList<OUT_DATA_TRADE_ROW>  OUT_DATA_TRADES = new ArrayList<OUT_DATA_TRADE_ROW>();
	}

	@ApiModel(value="OUT_DATA_ROW-BR_UPBIT_EXCHANGE_GET_ORDER")
	@Data
	...
	@ApiModel(value="OUT_DATA_TRADE_ROW-BR_UPBIT_EXCHANGE_GET_ORDER")
	@Data
	static class OUT_DATA_TRADE_ROW {

		@JsonProperty("MARKET")  //String
		@Schema(name = "MARKET", example = "1", description = "마켓의 유일 키")
		String MARKET = null;

		@JsonProperty("UUID")  //String
		@Schema(name = "UUID", example = "1", description = "체결의 고유 아이디")
		String UUID = null;

		@JsonProperty("PRICE")
		@Schema(name = "PRICE", example = "KRW-BTC", description = "체결 가격")
		String PRICE = null;

		@JsonProperty("VOLUME")
		@Schema(name = "VOLUME", example = "BTC/KRW", description = "체결 양")
		String VOLUME = null;

		@JsonProperty("FUNDS")
		@Schema(name = "FUNDS", example = "limit", description = "체결된 총 가격")
		String FUNDS = null;

		@JsonProperty("SIDE")
		@Schema(name = "SIDE", example = "ask,bid", description = "체결 종류")
		String SIDE = null;
	...
	@Autowired
	SA_UPBIT_EXCHANGE_GET_ORDER saUpbitExchangeGetOrder;
	...
	public OUT_DS run(@RequestBody IN_DS inDS) throws BizException {
		...
		HashMap<String, Object> h;
			try {
				h = saUpbitExchangeGetOrder.run( UUID,IDENTIFIER);
				...
				if(h!=null) {
					...
					ArrayList<HashMap<String, Object>> t = (ArrayList<HashMap<String, Object>>)  h.get("trades");
					for(int i=0;i<t.size();i++){
						HashMap<String, Object> tmp = t.get(i);
						OUT_DATA_TRADE_ROW row_trade = new OUT_DATA_TRADE_ROW();
						row_trade.MARKET = tmp.get("market").toString();
						row_trade.UUID = tmp.get("uuid").toString();
						row_trade.PRICE = tmp.get("price").toString();
						row_trade.VOLUME = tmp.get("volume").toString();
						row_trade.FUNDS = tmp.get("funds").toString();
						
						row_trade.SIDE = tmp.get("side").toString();
						row_trade.CREATED_AT = tmp.get("created_at").toString();

						outDs.OUT_DATA_TRADES.add(row_trade);
					}
