...
public class DA_STCK_BUY {
	...
	public Page<StBuy> findStckBuy(Pageable p) {
		JPAQuery<StBuy> c= qf.selectFrom(QStBuy.stBuy)
				.where(QStBuy.stBuy.delYn.eq("N"));
		c= c.offset(p.getOffset()); // offset과
		c= c.limit(p.getPageSize()); // Limit 을 지정할 수 있고
		c= c.orderBy(QStBuy.stBuy.buyDate.asc());
		QueryResults<StBuy> result= c.fetchResults();
		/*
